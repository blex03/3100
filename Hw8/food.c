...
void add_to_buffer(int item, int col, two_d_buffer *p, int id)
{
	...
	while(p->counts[col] >= p->size){
		//printf("pid: %d producer sleeping\n", id);
		pthread_cond_wait(&p->consume_cond, &p->mutex);
		//printf("pid: %d producer awake\n", id);
	}

	if (p->remain <= 0){
		//printf("EXITING WITH %d REMAINING", p->remain);
		p->remain -= 1;
		return;
	}
	...
	//printf("pid: %d waking up consumer\n", id);
	pthread_cond_signal(&p->produce_cond);
	
}

void remove_from_buffer(int *a, int *b, int *c, two_d_buffer *p, int id)
{
	//TODO
	//fill in code below
	pthread_mutex_lock(&p->mutex);
	//printf("cid: %d removing\n", id);
	
	while(p->counts[0] <= 0 || p->counts[1] <= 0 || p->counts[2] <= 0){
		//printf("cid: %d consumer sleeping\n", id);
		pthread_cond_wait(&p->produce_cond, &p->mutex);
		//printf("cid: %d consumer awake\n", id);
	}
	...
	*c = p->buf[0][2];
	...
	//printf("cid: %d waking up producer\n", id);
...
{   
	...
	while(!done)
	{
		...
		add_to_buffer(p->head->v, p->head->v, my_data->q, my_data->id);

		//printf("                          PID: %d ADDED TO BUFFER WITH %d REMAINING\n", my_data->id, q->remain);

		if (q->remain >= 0){
			my_data->total += 1;
		}

		node *new = create_node(p->head->v);

		add_last(&p->head, &p->tail, new);
		free(remove_first(&p->head, &p->tail));

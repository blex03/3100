...
void * thread_c(void * arg_in)
{
    ...
    while (guess_status != 0){
        ...
        guess_status = gmn_check(&gmn, arg->guess);
        arg->result = guess_status;
        pthread_cond_signal(&arg->cond_result);
    }

    memcpy(arg->message, gmn_get_message(&gmn), sizeof(arg->message));
...
int main(int argc, char *argv[])
{
    ...
    pthread_create(&tid1, NULL, thread_c, &arg);
    pthread_create(&tid2, NULL, thread_p, &arg);
